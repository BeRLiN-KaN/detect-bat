...
im = imread('Batt03.jpg'); % circles are greater than rectangles
%im = imread('objects.jpg'); % circles and rectangles are equal 

[h, w, ~] = size(im); % get height and width of image

distR = double(im(:, :, 3)) - 1; % since we have only one dimension, first dimension of image will be enough
...
%         hText = text(dimensions(1) + dimensions(3)/2, ...
%             dimensions(2)+dimensions(4)/2,inserted_text,'Color',[1 0 1],'FontSize',15); %put the pink string to image
%     elseif dimensions(3)~= dimensions(4)
%         rec_num = rec_num+1;
%         hText = text(dimensions(1) + dimensions(3)/2, ...
%             dimensions(2)+dimensions(4)/2,inserted_text,'Color',[0.1 0 0.5],'FontSize',15); %put the blue string to image
%     end
    
    rectangle('Position', stats(i).BoundingBox, 'EdgeColor', 'g', 'LineWidth', 1); % draw a rectangle
